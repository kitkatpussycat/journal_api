...
RSpec.describe "Categories", type: :request do
  describe "GET /categories" do
    it 'returns all categories' do
      ...
      c1 = FactoryBot.create(:category, name: 'name', description: 'this is a description', user:user)
      c2 = FactoryBot.create(:category, name: 'name', description: 'this is another description', user:user)
    ...
    it 'returns a category' do
      ...
      auth_headers = Devise::JWT::TestHelpers.auth_headers(headers, user)
      cat = FactoryBot.create(:category, name: 'name', description: 'this is a description', user:user)
  ...
  describe "POST /categories" do
    it 'creates a new category' do
      ...
      auth_headers = Devise::JWT::TestHelpers.auth_headers(headers, user)
      expect {
        post '/api/v1/categories', headers: auth_headers, params: {category: {name: 'name', description: 'this is a description', user_id: user.id}}.to_json
  ...
  describe "PATCH/PUT /categories/:id" do
    ...
    it 'should update the category data' do
      ...
      auth_headers = Devise::JWT::TestHelpers.auth_headers(headers, user)
      cat = FactoryBot.create(:category, name: 'name', description: 'description', user:user)
      put "/api/v1/categories/#{cat.id}", params: {category: {name: 'name', description: 'description'}}.to_json, headers: auth_headers

      expect(response).to have_http_status(200)
      json_body = JSON.parse(response.body)
      expect(json_body['name']).to eq('name')
      expect(json_body['description']).to eq('description')
    end
  end

  describe 'DELETE /categories/:id' do
    let!(:user) { FactoryBot.create(:user, email: 'email@email.com', password:'password')}
    let!(:cat) { FactoryBot.create(:category, name: 'name', description: 'this is a description', user: user) }
